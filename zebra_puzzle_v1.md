...
### 2.2. Knuth's X Algorithm using a sparse matrix
...
### 2.3. Sudoku as an example of a set coverage problem

Before we get into the actual goald of tnis project - solving a Zebra Puzzle - let's quickly revisit how a Sudoku can be solved using Knuth's agorithm. Sudoku can be modelled as an exact cover problem[^4][^4.2] and represetned a a sparse matrix for the X algorithm as follows:

- **elements** the Universe $U$ are representing specific **constraints** of the sudoku puzzle. These are:
  - constraints on rows, generated by the rule "*each digit can appear only once in each row*":
    - "there is exaclty one `1` in the first row"
    - "there is exaclty one `2` in the first row"
    - "there is exaclty one `3` in the first row"
    - ...
    - "there is exaclty one `9` in the ninth row"
  - constraints on columns, generated by the rule "*each digit can appear only once in each column*":
    - "there is exaclty one `1` in the first column"
    - "there is exaclty one `2` in the first column"
    - "there is exaclty one `3` in the first column"
    - ...
    - "there is exaclty one `9` in the ninth column"
  ...
  - constraints due to each position in grid being allowed to hold only one digit:
    - "the grid position (1,1) has exactly one in it
    - "the grid position (1,2) has exactly one in it
    - ...
    - "the grid position (9,9) has has exactly one in it
...
- additionally we can add explicit cosntraints from the specific puzzle's text.

Using the above algorithm a sparse matrix representing a sudoku puzlle can be cosntucted. It is (without specific puzzle text constraitns) a $729 \times 324$ matrix that's hard to put whole in this documetn, but it's snippets would look like this:
